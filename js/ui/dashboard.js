...
export function renderDashboard(ctx) {
  ...
  return el('div', {},
    ...
    !state.job ? renderNudge(ctx) : null,
  );
}
...
function renderGoalCard(ctx) {
  ...
  return el('div', { class: 'card mb' },
    el('div', { class: 'row between' },
      el('h2', {}, challenge.emoji + ' ' + challenge.title),
...
function renderAccountsGrid(ctx) {
  ...
  const a = state.accounts;
  const cards = [
    acctCard('💳 Checking', a.checking, 'Money for everyday spending', a.checking < 0 ? 'overdraft' : null, 'checking-account'),
    acctCard('🏦 Savings', a.savings, pct(state.rates.savingsApy) + ' APY', null, 'savings-account'),
    acctCard('🌱 High-Yield Savings', a.hysa, pct(state.rates.hysaApy) + ' APY', null, 'high-yield-savings'),
    acctCard('🔒 Retirement', a.retirement,
  ...
  if (state.card) {
    ...
    cards.push(acctCard('💥 Credit Card', -state.card.balance,
  ...
  if (debt > 0) {
    cards.push(acctCard('⚖️ Debts', -debt, state.debts.filter((d) => d.principal > 0).length + ' open', null, 'debt'));
  }
  return el('div', { class: 'grid autofit' }, cards);
}

function acctCard(label, balance, sub, warnBadge, glossaryId) {
  return el('div', { class: 'card acct' + (balance < 0 ? ' negative' : '') },
    el('div', { class: 'label' }, label, glossaryId ? whyButton(glossaryId) : null,
    ...
    score == null
      ? el('div', {},
          ...
          canHaveCard(state)
            ...
            : el('p', { class: 'tiny' }, 'Credit unlocks at 18 in this run. For now, savings habits are your superpower.'))
      : el('div', {},
          el('div', { class: 'gauge' },
...
function renderActivityCard(state) {
  ...
  const icons = {
    income: '💵', spend: '🛍️', save: '🏦', interest: '✨', fee: '💸',
    debt: '⚖️', card: '💳', event: '🎲', tax: '🏛️',
  };
  return el('div', { class: 'card' },
    ...
    recent.length === 0
      ...
      : recent.map((t) => el('div', { class: 'txn' },
          el('span', { class: 'dot' }, icons[t.type] || '•'),
          el('span', { class: 'what' },
...
function renderNudge(ctx) {
  return el('div', { class: 'card mt', style: 'border-color: var(--brand)' },
    el('h3', {}, '👋 First step: get money flowing'),
